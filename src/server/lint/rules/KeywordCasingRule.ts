...
function toWords(text: string) {
    const tokens = text.toLocaleUpperCase().split(" ");
    const words: IWords = {};
        ...
        new class extends BaseRuleWalker {
            public visitTerminal(node: TerminalNode): void {
                ...
                if (
                    ...
                    (
                        (
                            upperText in KeywordCasingRule.stdWords ||
                            upperText in KeywordCasingRule.obdcWords ||
                            upperText in KeywordCasingRule.futureWords
                        ) ||
                        (
                            isDataType &&
                            symbol.type === TSqlParser.ID
                        )
                    )
                ) {
                    ctx.addError(
